...
KHASH_INIT(int16map, uint16_t, uint8_t, 1, kh_int_hash_func, kh_int_hash_equal);
                  ...
                  uint8_t subscriber)
{
  ...
  union hash_key_union key = { .hashKey = { .type = type,
                                            .property = property } };

  khint_t it = kh_get_int16map(map->map, key.intKey);
                ...
                enum hash_value_enum value)
{
  ...
  if(it == kh_end(map->map)) {
    // Insert the new key.
    int ret = 0;
    it = kh_put_int16map(map->map, key.intKey, &ret);
    switch(ret) {
      case -1:
